...
if __name__ == '__main__':
    ...
    input_vehicle = 'KAAD19_PASSENGER_VEHICLE.csv'
    input_yehu = 'KAAD21_PASSENGER_YEHU.csv'
    input_bus = 'LONGDISTANCE_BUS.csv'

    output_file = 'sj_gps_offline'

    etc_path = 'C:/Users/Administrator/Desktop/lkyw/ETC'
    ...
    sql_zlzgxx = 'SELECT ZGKSRQ, ZGJSRQ FROM ZFXW_ZLZGXX'
    ...
    data_zlzgxx = pd.read_sql(sql_zlzgxx, con=mysql_engine.connect())
    ...
    output_data = output_data[['CERTIFICATE_NO', 'VEHICLE_NO', 'PLATE_COLOR', 'NAME', 'AREA', 'GPS_DEVICE_MODEL',
                               'GPS_PLATFROM_NAME', 'BUS_ID', 'DRVDATE']]

    output_data.rename(columns={'CERTIFICATE_NO': 'transport_certificate', 'VEHICLE_NO': 'vehicle_no',
                                'PLATE_COLOR': 'plate_color', 'NAME': 'company_name', 'AREA': 'registered_area',
                                'GPS_DEVICE_MODEL': 'terminal_type', 'GPS_PLATFROM_NAME': 'terminal_platform',
                                'BUS_ID': 'longdistance_bus_id', 'DRVDATE': 'departure_date'}, inplace=True)
    ...
    output_data['no_gps_days'] = '1'
    output_data['last_gps_date'] = ''

    output_data.to_csv(os.path.join(output_history_path, output_file + '_' + today + '.csv'), index=False)
